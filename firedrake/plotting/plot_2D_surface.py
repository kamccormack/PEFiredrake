...
sys.path.append('/fenics/shared/forward_codes/')
...
resultspath = '/fenics/shared/forward_codes/results/numpy_results/'
variablespath = '/fenics/shared/forward_codes/results/numpy_variables/'
...
gps_file = '/fenics/shared/data/GPS_displacement.xlsx'
...
def plot_2D_3D_compare():
	...
	end3D = np.where(dtt_all3D / days > endday)[0][0]
	...
	for i in range(0, loop_through.shape[0]):
		...
		for ind in cross_ind:
			...
			ind_3Dmodel = (np.abs(np.sqrt(pow((X_gps_3Dmodel[:] - X_gps_post[ind]), 2) +
			                            pow((Y_gps_3Dmodel[:] - Y_gps_post[ind]), 2)))).argmin()

			ind_3D_p = (np.abs(np.sqrt(pow((x_surf_3D[:] - X_gps_post[ind]), 2) +
			                            pow((y_surf_3D[:] - Y_gps_post[ind]), 2)))).argmin()
			
			#print out locations of GPS, 2D and 3D results
			print ('Xgps: ',X_gps_post[ind], 'X2D: ', x_surf_ux[idx_u], 'X3D: ', X_gps_3Dmodel[ind_3Dmodel])
			print ('Ygps: ',Y_gps_post[ind], 'Y2D: ', loop_through[i], 'X3D: ', Y_gps_3Dmodel[ind_3Dmodel])
			...
			Sol_surf_x -= Sol_surf_x[idx_u, 0] - Ux_post[0]
			Sol_surf_y -= Sol_surf_y[idx_u, 0] - Uz_post[0]
			Ux_gps_model_all -= Ux_gps_model_all[ind_3Dmodel, 0] - Ux_post[0]
			Uz_gps_model_all -= Uz_gps_model_all[ind_3Dmodel, 0] - Uz_post[0]
			...
			for j in range(0, time_post.shape[0]):
				t_ind2D = np.abs(np.sqrt((dttnew - time_post[j]) ** 2)).argmin()
				t_ind3D = np.abs(np.sqrt((dtt_days_3D - time_post[j]) ** 2)).argmin()
				Ux_post_norm2D.append(Ux_post[j] - (Sol_surf_x[idx_u, t_ind2D] - Sol_surf_x[idx_u, 0]))
				Uz_post_norm2D.append(Uz_post[j] - (Sol_surf_y[idx_u, t_ind2D] - Sol_surf_y[idx_u, 0]))
				if j > 0:
					xper2D = 1e2 * (Sol_surf_x[idx_u, t_ind2D] - Sol_surf_x[idx_u, 0]) / (Ux_post[j] - Ux_post[0])
					xper3D = 1e2 * (Ux_gps_model_all[ind_3Dmodel, t_ind3D] - Ux_gps_model_all[ind_3Dmodel, 0]) / (Ux_post[j] - Ux_post[0])
					if xper2D >= 100.:
						# print "percent error x", names[ind], xper
						# print "numer = ",Sol_surf_x[idx_u, norm_i] - Sol_surf_x[idx_u, 0]
						# print "denominator = ", Ux_post[j] - Ux_post[0]
						Uxpercent2D.append(100.)
					elif xper2D <= -100.:
						Uxpercent2D.append(-100.)
					else:
						Uxpercent2D.append(xper2D)
					if xper3D >= 100.:
						Uxpercent3D.append(100.)
					elif xper3D <= -100.:
						Uxpercent3D.append(-100.)
					else:
						Uxpercent3D.append(xper3D)

					
					zper2D = 1e2 * (Sol_surf_y[idx_u, t_ind2D] - Sol_surf_y[idx_u, 0]) / (Uz_post[j] - Uz_post[0])
					zper3D = 1e2 * (Uz_gps_model_all[ind_3Dmodel, t_ind3D] - Uz_gps_model_all[ind_3Dmodel, 0]) / (Uz_post[j] - Uz_post[0])

					if zper2D >= 100.:
						# print "percent error z", names[ind], zper
						# print "numer = ",Sol_surf_y[idx_u, norm_i] - Sol_surf_y[idx_u, 0]
						# print "denominator = ", Uz_post[j] - Uz_post[0]
						Uzpercent2D.append(100.)
					elif zper2D <= -100.:
						Uzpercent2D.append(-100.)
					else:
						Uzpercent2D.append(zper2D)
					if zper3D >= 100.:
						Uzpercent3D.append(100.)
					elif zper3D <= -100.:
						Uzpercent3D.append(-100.)
					else:
						Uzpercent3D.append(zper3D)
			...
			ax1.plot(dttnew / days, Sol_surf_p[idx_p, :], 'blue', label = '2D', linewidth=3)
			ax1.plot(dtt_days_3D, Sol_surf_p3D[ind_3D_p, :], '--b', label = '3D', linewidth=3)

			
			ax2.plot(dttnew[0:end] / days, Sol_surf_x[idx_u, 0:end], 'green', label='model',
			         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
			ax2.plot(dtt_days_3D, Ux_gps_model_all[ind_3Dmodel, :], '--g', label='3D model', linewidth=3, fillstyle='none')
			#ax2.plot(dtt_days_3D/10, Ux_gps_model_all[ind_3Dmodel, :], '--g', linewidth=1, fillstyle='none')
			ax2.plot(time_post, Ux_post, '.k', label='data', linewidth=3)  # , fillstyle='none')
			
			
			#ax2.plot(time_post, Ux_post_norm, '.g', label='residual', linewidth=3, fillstyle='none')
			
			ax3.plot(dttnew[0:end] / days, Sol_surf_y[idx_u, 0:end], 'red', label='2D model',
			         linewidth=3)  # ,ms=1, mew=1.5, fillstyle='none') #pressure
			ax3.plot(dtt_days_3D, Uz_gps_model_all[ind_3Dmodel, :], '--r', label='3D model', linewidth=3, fillstyle='none')
			#ax3.plot(dtt_days_3D/10, Uz_gps_model_all[ind_3Dmodel, :], '--r', linewidth=1, fillstyle='none')
			ax3.plot(time_post, Uz_post, '.k', label='data', linewidth=3)  # , fillstyle='none')
			...
			ax1.set_title('GPS station: '+names[ind])
			fig.savefig('figures/2D_crosstimeseries_cross%s_%s.png'
			            % (str(loop_through[i])[:3], str(X_gps_post[ind])[:3]),dpi=400)
			
			fig = plt.figure()
			ax1 = fig.add_subplot(211)
			ax2 = fig.add_subplot(212)

			ax1.plot(time_post, Uxpercent2D, '.g', label = '2D')
			ax2.plot(time_post, Uzpercent2D, '.r')
			
			ax1.plot(time_post, Uxpercent3D, '*g', label = '3D')
			ax2.plot(time_post, Uzpercent3D, '*r')
			...
			ax1.set_ylim([np.asarray(Uxpercent2D).min() - 10., np.asarray(Uxpercent2D).max() + 10.])
			ax2.set_ylim([np.asarray(Uzpercent2D).min() - 10., np.asarray(Uzpercent2D).max() + 10.])

			fig.savefig('figures/2D_cross_percent_%s_%s.png' % (str(loop_through[i])[:3], str(X_gps_post[ind])[:3]),
			            dpi=400)
			#
		fig = plt.figure()
		ax1 = fig.add_subplot(211)
