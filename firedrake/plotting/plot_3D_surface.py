...
Sol_surf_p = 1e3/(9.81)*np.load(results_path+"Sol_surfp_3D_EQ_mapped.npy")

print(Sol_gps_mapped.shape, Sol_gps_mapped.max(), Sol_gps_mapped.min())
print(gps_dofs.shape)
#quit()


x_surf, y_surf = x_all[surface_dofs], y_all[surface_dofs]
...
#gpsdim = int(gps_dofs.shape[0]/3)

#solution at GPS stations
#gps_dofs_xy = gps_dofs[0:gpsdim].astype(int)

#Sol_gps_mapped = Sol_all_mapped[gps_dofs.astype(int), :]

X_gps_model, Y_gps_model = x_all[gps_dofs], y_all[gps_dofs]

#print(X_gps_model, Y_gps_model, z_all[gps_dofs])

#quit()


Ux_gps_model = Sol_gps_mapped[:, 0, 0]
Uy_gps_model = Sol_gps_mapped[:, 1, 0]
Uz_gps_model = Sol_gps_mapped[:, 2, 0]

Ux_gps_model_all = 1e2*Sol_gps_mapped[:, 0, :]
Uy_gps_model_all = 1e2*Sol_gps_mapped[:, 1, :]
Uz_gps_model_all = 1e2*Sol_gps_mapped[:, 2, :]
...
#topoflux = 9.5e5 #m3/day
topoflux = 3.83e5 #m3/day
subflux = 4.3e4 #m3/day
#subflux = 0. #m3/da

sea_flux_tec = np.concatenate([[subflux, subflux], sea_flux[:]+subflux])
sea_flux_topo = topoflux*np.ones(sea_flux_tec.shape[0])

#print( sea_flux_topo.max(), sea_flux_tec.max())
...
#dtt_all = np.concatenate([[-1e8, -1],np.cumsum(dt)]) #- 5256e2*50*60 #timeline

dtt_days = dtt_all/(3600*240)


#seaflux_cum = np.cumsum(sea_flux*dtt_days)

#tt = np.where(dtt_days > 30)[0][0]

#print (seaflux_cum[tt])
print (np.where(dtt_days > 360)[0][0])
#quit()
...
def plot_gps_u():
	days = 1.
	endday = 40
	end = np.where(dtt_days / days > endday)[0][0]
	...
	for ind in ind_gps_time:
		...
		#ind_model = np.abs(x_surf_ux[:] - X_gps_post[ind]).argmin()
		...
		# Calculate the "normalized" GPS data and the percentage of data explained by PE
		for j in range(0, time_post.shape[0]):
			norm_i = np.abs(np.sqrt((dtt_days - time_post[j]) ** 2)).argmin()
			Ux_post_norm.append(Ux_post[j] - (Ux_gps_model_all[ind_model, norm_i] - Ux_gps_model_all[ind_model, 0]))
			Uy_post_norm.append(Uy_post[j] - (Uy_gps_model_all[ind_model, norm_i] - Uy_gps_model_all[ind_model, 0]))
			Uz_post_norm.append(Uz_post[j] - (Uz_gps_model_all[ind_model, norm_i] - Uz_gps_model_all[ind_model, 0]))
			if j > 0:
				xper = 1e2 * (Ux_gps_model_all[ind_model, norm_i] - Ux_gps_model_all[ind_model, 0]) / (Ux_post[j] - Ux_post[0])
				if xper >= 100.:
				...
				yper = 1e2 * (Uy_gps_model_all[ind_model, norm_i] - Uy_gps_model_all[ind_model, 0]) / (Uy_post[j] - Uy_post[0])
				if yper >= 100.:
					Uypercent.append(100.)
				elif yper <= -100.:
					Uypercent.append(-100.)
				else:
					Uypercent.append(yper)

				zper = 1e2 * (Uz_gps_model_all[ind_model, norm_i] - Uz_gps_model_all[ind_model, 0]) / (Uz_post[j] - Uz_post[0])
				if zper >= 100.:
		...
		print(ind, names[ind], gps_time[ind_time])
		...
		#
		# sol_plotgps[0].plot(dtt_days[0:endtime], Ux_gps_model_all[idx_gps, 0:endtime], '--k', label='constant k', linewidth=3, fillstyle='none') #pressure
		#
		...
		sol_plotgps[1].plot(dtt_days, Uy_gps_model_all[ind_model, :], 'r', linewidth=3, fillstyle='none')

		sol_plotgps[2].plot(dtt_days, Uz_gps_model_all[ind_model, :], 'g', linewidth=3, fillstyle='none')


		sol_plotgps[0].plot(time_post , Ux_post, '.k', linewidth=3)#, fillstyle='none')

		sol_plotgps[1].plot(time_post , Uy_post, '.r', linewidth=3)#, fillstyle='none')

		sol_plotgps[2].plot(time_post , Uz_post, '.g', linewidth=3)#, fillstyle='none')

		sol_plotgps[0].plot(time_post , Ux_post_norm, '.k', linewidth=3, fillstyle='none')

		sol_plotgps[1].plot(time_post , Uy_post_norm, '.r', linewidth=3, fillstyle='none')

		sol_plotgps[2].plot(time_post , Uz_post_norm, '.g', linewidth=3, fillstyle='none')
		...
		#
		sol_plotgps[0].set_ylim([Ux_post.min()-1. , Ux_post.max()+2.])
		sol_plotgps[1].set_ylim([Uy_post.min()-1. , Uy_post.max()+1.])
		sol_plotgps[2].set_ylim([Uz_post.min()-1. , Uz_post.max()+1.])

		# sol_plotgps[0].set_ylim([Ux_gps_model_all[ind_model,:].min()-.2 , Ux_gps_model_all[ind_model,:].max()+.2])
		# sol_plotgps[1].set_ylim([Uy_gps_model_all[ind_model,:].min()-.2 , Uy_gps_model_all[ind_model,:].max()+.2])
		# sol_plotgps[2].set_ylim([Uz_gps_model_all[ind_model,:].min()-.2 , Uz_gps_model_all[ind_model,:].max()+.2])

		sol_plotgps[0].set_xlim([0, endday])
		sol_plotgps[1].set_xlim([0, endday])
		sol_plotgps[2].set_xlim([0, endday])
		...
		ax1.plot(time_post, Uxpercent, '.k')
		ax2.plot(time_post, Uypercent, '.r')
		ax3.plot(time_post, Uzpercent, '.g')


		# plt.ylabel('Depth (km)')
		ax1.set_ylabel('Percent trench-ward')
		ax2.set_ylabel('Percent Y disp')
		ax3.set_ylabel('Percent uplift')
		...
		ax2.set_ylim([np.asarray(Uypercent).min() - 10., np.asarray(Uypercent).max() + 10.])
		ax3.set_ylim([np.asarray(Uzpercent).min() - 10., np.asarray(Uzpercent).max() + 10.])

		fig.set_size_inches(8, 8)


		fig.savefig('figures/3D_percent_%s.png' % ind, dpi=400)
...
def plot_gps(size_gps, n):
	...
	for i in range(0,n):
		...
		ax1.plot(Sol_gps_mapped) #pressure
		...
		fig.savefig('figures/Sol_gpstest.png' , dpi=400)

def plot_surf(points, splice):

	#points = 1e3*np.array([100 , 130])

	for i in range(0, points.shape[0]):
		idx_p = np.abs(np.sqrt(np.power((x_surf[:] - points[i,0]),2) + np.power((y_surf[:] - points[i,1]),2))).argmin()
		fig = plt.figure()
		ax1 = fig.add_subplot(111)

		ax1.plot(dtt_days/splice, Sol_surf_p[idx_p, :], '-k', label='mapped k', linewidth=3, fillstyle='none') #pressure
		...
		fig.savefig('figures/3d_surf_time_%s_3yr_well.png'% str(points[i,0])[:3], dpi = 400)
		#fig.savefig('figures/3d_surf_time_%s_60day_well.png'% str(points[i,0])[:3], dpi = 400)
...
def plot_surf_k(points, splice):
	...
	for i in range(0, points.shape[0]):
		...
		fig.set_size_inches(8, 2.5)
		plt.xlabel('Days after EQ')
		fig.savefig('figures/3d_surf_timeseries_%s_3yr_well.png'% str(points[i,0])[:3], dpi = 400)
...
def plot_sea_flux():
	...
	ax1.semilogy(dtt_days/splice, sea_flux_tec, '-r', label='Tectonic flow', linewidth = 2.5)
	ax1.semilogy(dtt_days/splice, sea_flux_topo, '-b',label='Topographic flow',  linewidth = 2)
	ax1.semilogy(dtt_days / splice, (8/7)*sea_flux_topo, '--b', label='Topographic flow', linewidth=1)
	ax1.semilogy(dtt_days / splice, (6/7)*sea_flux_topo, '--b', label='Topographic flow', linewidth=1)
...
#points = 1e3*np.array([[75, 136], [94, 125], [103, 140], [130,135]])
points = 1e3*np.array([[92,150]])


splice = 365
endtime = 300

#plot_surf(points, splice)
#plot_surf_k()
